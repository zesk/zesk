...
class Test_Generator extends Options {
	...
	public function run(): void {
		...
		while (($arg = $this->getArgument('target')) !== null) {
		...
		if (count($dirs) + count($files) === 0) {
			$this->verboseLog("Generating tests for the current directory: $cwd\n");
			$dirs[] = $cwd;
		}
		$dry_run = $this->optionBool('dry-run');
		if ($dry_run) {
			$this->verboseLog("Dry run: No files will be created.\n");
		}
		$extensions = ArrayTools::prefixValues(ArrayTools::valuesRemovePrefix('.', $this->optionIterable('extensions')), '.');
		foreach ($dirs as $dir) {
			$this->verboseLog("Processing directory $dir ...");
			...
			foreach ($dir_files as $fileInfo) {
				...
				$file = path($dir, $file);
				$this->verboseLog("Processing $file ...\n");
				...
				if (!is_dir($dest_path)) {
					if (!$dry_run) {
						...
						$this->verboseLog("Created directory $dest_path ...\n");
					} else {
		...
		foreach ($files as $file) {
			$this->verboseLog("Processing file $file ...\n");
			if (!Directory::isAbsolute($file)) {
				$file_full = path($cwd, $file);
			}
			if (!StringTools::ends($file, $extensions)) {
				$this->verboseLog("Skipping $file because extension doesn't match");
