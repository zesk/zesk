...
class Test extends Hookable {
	...
	final public function load_module($module) {
		...
		$this->assertTrue($app_module->loaded($module), "Module $module is not found");
	 ...
	 * @param string $message
	 */
	final public function assertFalse($condition, $message = null) {
	 ...
	 * @param string $message
	 */
	final public function assertTrue($condition, $message = null): void {
	 ...
	 * @param string $message
	 */
	final public function assertIsString($mixed, $message = null): void {
	 ...
	 * @param string $message
	 */
	final public function assertIsInteger($mixed, $message = null): void {
	...
	final public function assert_class_exists($class, $message = null): void {
		$this->assertIsString($class, 'Class passed to ' . __METHOD__ . ' should be string');
	 ...
	 * @param string $message
	 */
	final public function assertNotNull($value, $message = null): void {
		if (!$message) {
	 ...
	 * @param string $message
	 */
	final public function assertNull($value, $message = null): void {
		$this->assertEquals(null, "$value === null : $message", false, $value);
	}
	 ...
	 */
	final protected function assertEquals($expected, $actual, $message = null, $strict = true): void {
		$this->assert(is_array($actual), gettype($actual) . ' is not an array');
		$this->assert(is_array($expected), gettype($expected) . ' is not an array');
		$this->assertEquals($expected, $actual, $message, $strict);
	}
	...
	final protected function assert_equal($expected, $actual, $message = null, $strict = true): void {
		...
		if (is_scalar($actual) && is_scalar($expected)) {
			if (is_float($actual) && is_float($expected)) {
				...
				$this->assertEquals($expected, $message, $actual);
			} else {
	...
	final protected function assertNotEquals($expected, $actual, $message = null, $strict = true): void {
		if ($message === null) {
	...
	final protected function assert_equal_array($expected, $actual, $message = '', $strict = true, $order_matters = false): void {
		...
		foreach ($actual as $k => $v) {
			...
			if (is_array($v)) {
				$this->assertEquals($v, $expected[$k], "[$k] $message", $strict);
			} elseif (is_object($v)) {
				$this->assertEquals(get_class($expected[$k], $v::class));
				$this->assertEquals($v, $expected[$k], "Comparing Key($k) => ");
			} elseif ($strict) {
				if ($v !== $expected[$k]) {
		...
		if ($order_matters) {
			$this->assertEquals(implode(';', array_keys($actual)), implode(';', array_keys($expected)), 'Ordering is different: ' . implode(';', array_keys($actual)) === implode(';', array_keys($expected)));
		}
	}
	...
	final protected function assert_array_contains($subset, $superset, $message = ''): void {
		...
		foreach ($subset as $k => $v) {
			...
			if (is_array($v)) {
				$this->assertEquals($v, $superset[$k], "$message: Key $k in array");
			} else {
				$this->assertEquals($v, "key $k: $superset[$k] !== $v", $superset[$k]);
			}
		}
	}
	...
	public static function run_one_class(Application $application, $class, array $options, &$object = null) {
		try {
			...
			throw new Exception_Invalid('Unable to create {class} - Exception {exceptionClass}: {message}', [
				'class' => $class,
	...
	private static function _find_test(Application $application, string $class): bool {
		...
		try {
			$include = File::findFirst(array_keys($application->autoloader->path()), $path);
	...
	private static function _configuration_load(Application $application): array {
		...
		$config = $configuration->getPath('zesk\\Test_Unit::config', $test_command->default_configuration_file('test'));
		if (!$config) {
			return [];
		}
		...
		if (toBool($configuration->getPath('zesk\\Command_Test::debug_config'))) {
