...
class Module_Health extends Module {
	...
	public function error_handler($errno, $errstr, $errfile, $errline) {
		...
		$type = strtolower(StringTools::removePrefix(self::$error_codes[$errno] ?? $errno, 'E_'));
		$this->log([
			'type' => $type,
			'code' => $errno,
			'fatal' => $fatal = self::$fatal_errors[$errno] ?? null,
			'message' => $errstr,
			'file' => $errfile,
			'line' => $errline,
	...
	private function purge_event_types($class, $date_column, array $where, Timestamp $when, $description): void {
		$delete = $this->application->ormRegistry($class)->queryDelete()->where($where + [
			"$date_column|<=" => $when,
		]);
		$delete->execute();
