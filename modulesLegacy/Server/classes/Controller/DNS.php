...
use Server\classes\Model\Model_DNS;
use zesk\ArrayTools;
...
class Controller_DNS extends Controller_Theme {
	...
	private function _compare_results($old, $new, $reverse = false) {
		...
		foreach ($old_result as $old_host => $old_record_type_list) {
			...
			foreach ($old_record_type_list as $old_record_type => $old_records) {
				$new_records = $new_record_type_list[$old_record_type] ?? null;
	...
	public function action_index($domain) {
		...
		// 		$control->addChild($this->widgetFactory(Control_Text::class)->names("old", "Old Server", true));
		// 		$control->addChild($this->widgetFactory(Control_Text::class)->names("new", "New Server", true));
		// 		$control->addChild($w = $this->widgetFactory(Control_Text::class)->textarea(true)->names("lookup", "Name", true));
		// 		$w->setOption('rows', 20);
		// 		$w->setOption('cols', 80);
