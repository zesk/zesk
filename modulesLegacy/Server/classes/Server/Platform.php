...
use Server\classes\Server\Exception\Server_Exception_Permission;
use Server\classes\Server\Exception\User\Server_Exception_User_NotFound;
use Server\classes\Server\Files\Server_Files_Direct;
use Server\classes\Server\Files\Server_Files_Simulate;
use Server\classes\Server\Server_Base;
...
abstract class Server_Platform extends Hookable {
	 ...
	 * @return boolean
	 */
	public function feature_exists($feature) {
		try {
	 ...
	 * @return number
	 */
	public function user_register($user, $group, $home = null, $shell = null, $uid = null) {
	...
	final public function feature($name) {
		return $this->features[strtolower($name)] ?? null;
	}
	 ...
	 * @return string
	 */
	final public function format_command($command) {
		$args = func_get_args();
		array_shift($args);
	...
	public function owner($path, $owner = null, $permissions = null): void {
		...
		$fuid = intval($owner['uid'] ?? null);
		$fgid = intval($owner['gid'] ?? null);
	...
	public function update_catenate($relative_file, array $paths, $dest, array $options = []) {
		...
		if (!$this->files->is_file($dest)) {
			$copy = true;
		} elseif ($this->files->md5_file($dest) !== $checksum) {
			$copy = true;
		}
		if (!$copy) {
	...
	abstract public function login_script_install($user, $name, $command);

	abstract public function login_script_installed($user, $name);

	abstract public function login_script_run($user, $name);
	...
	final public function database_beforeConfigure($urls): void {
		...
		foreach ($urls as $url) {
			try {
				$db = $this->application->databaseRegistry($url);
