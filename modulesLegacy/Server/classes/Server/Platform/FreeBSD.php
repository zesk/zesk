...
use Server\classes\Server\Packager\Server_Packager_PKG;
	...
	public function user($user) {
		...
		try {
			...
			return self::$users[$user] = ArrayTools::rekey([
				self::f_user_name, 'x-password', self::f_user_id, self::f_user_group_id, 'x-class', 'x-change',
				'x-expire', self::f_user_full_name, self::f_user_home, self::f_user_shell,
			], explode(':', $result, 9));
	...
	public function group($group) {
		...
		try {
			...
			$data = ArrayTools::rekey([
				self::f_group_name, null, self::f_group_id, self::f_group_members,
			], explode(':', $result, 4));
			...
			if (count($members) > 0) {
				foreach ($members as $member) {
					if (!$this->group_exists($member)) {
						throw new Server_Exception_Group_NotFound($members, "When adding $group with members {members}", ['members' => $members]);
					}
				}
				$ma = ' -m {1}';
