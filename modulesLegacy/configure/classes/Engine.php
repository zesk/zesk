...
class Engine extends Hookable {
	...
	public const OPTION_PROMPT = 'prompt';
	...
	public function __construct(Application $application, array $options = []) {
		...
		$this->variable_map['user'] = $username = $_SERVER['USER'] ?? null;
	 ...
	 * @return mixed
	 */
	public function prompt(string $message, string $default = null, array $completions = []): string|bool {
		if (!$this->prompt) {
			return $this->option(self::OPTION_PROMPT, true);
		}
	...
	public function command_mkdir($target, $flags) {
		...
		$result = $this->handle_owner_mode($target, $flags['owner'] ?? null, $flags['mode'] ?? null);
		if (is_bool($result)) {
			return $result;
		}
		return $changed;
	...
	public function command_file($source, $destination) {
		...
		$want_owner = $flags['owner'] ?? null;
		$want_mode = $flags['mode'] ?? null;
		$map = $flags['map'] ?? null;
		$trim = $flags['trim'] ?? null;
