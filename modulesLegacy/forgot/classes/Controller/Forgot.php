...
class Controller_Forgot extends Controller_Theme {
	...
	public function action_status($token) {
		...
		if (!Forgot::valid_token($token)) {
			$this->response->setStatus(HTTP::STATUS_BAD_REQUEST, 'Invalid token');
		...
		if (!$forgot) {
			$this->response->setStatus(HTTP::STATUS_FILE_NOT_FOUND, 'Token not found');
			return $this->json($json + [
		...
		if (!$user instanceof User) {
			$this->response->setStatus(HTTP::STATUS_FILE_NOT_FOUND, 'User not found');
			return $this->json($json + [
				'type' => 'user-not-found',
		...
		if ($forgot->id() !== $session->forgot) {
			$this->response->setStatus(HTTP::STATUS_EXPECTATION_FAILED, 'Session mismatch');
			return $this->json($json + [
		...
		if (!$forgot->memberIsEmpty('updated')) {
			$this->response->setStatus(HTTP::STATUS_UNPROCESSABLE_ENTITY, 'Already used');
			return $this->json($json + [
				'type' => 'already',
