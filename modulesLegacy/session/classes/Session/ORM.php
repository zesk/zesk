 ...
 * @author kent
 */
class Session_ORM extends ORMBase implements Interface_Session {
	/**
	 ...
	 * @param Application $application
	 * @param string $hash
	 * @return \zesk\ORMBase|null
	 */
	public static function one_time_find(Application $application, string $hash): ?ORMBase {
		$hash = trim($hash);
	...
	public function session_count($nSeconds = 600) {
		...
		$where['id|!='] = $this->id();
		return $this->querySelect()->what('*X', 'COUNT(id)')->where($where)->one_integer('X');
	}
