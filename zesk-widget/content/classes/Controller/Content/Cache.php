...
class Controller_Content_Cache extends Controller_Cache {
	...
	public static function cache_prefix(Configuration $configuration) {
		return $configuration->getPath(__CLASS__ . '::cache_prefix', '/cache/image/');
	}
	...
	public static function image_changed(Content_Image $image): void {
		...
		$path = path($app->documentRoot(), self::cache_prefix($app->configuration), $image->id());
		if (is_dir($path)) {
	...
	protected function apply_commands(array $commands, $data) {
		...
		foreach ($commands as $command) {
			...
			$command['data'] = $data;
			$new_data = $this->callHookArguments('image_' . $hook, [
				$command,
			], null);
			if ($new_data) {
				$data = $new_data;
	 ...
	 * @return string
	 */
	protected function hook_imageScale(array $command) {
		$width = $height = null;
		extract($command, EXTR_IF_EXISTS);
		return Image_Library::factory($this->application)->imageScaleData($command['data'], [
			'zoom' => true,
			'width' => $width,
			'height' => $height,
		]);
