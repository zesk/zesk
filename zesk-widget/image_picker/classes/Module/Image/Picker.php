...
class Module_Image_Picker extends Module_JSLib implements Interface_Module_Routes {
	...
	public function hook_routes(Router $router): void {
		$router->addRoute('imagepicker', [
			'controller' => Controller_Image_Picker::class,
		]);
		$router->addRoute('imagepicker/image/{zesk\Content_Image image}/delete', [
			'controller' => Controller_Image_Picker::class,
			'action' => 'image_delete',
			'arguments' => [
				2,
		...
		$router->addRoute('image_picker/{option action}', [
			'controller' => Controller_Image_Picker::class,
		]);
	}
	...
	protected function hook_configured(): void {
		try {
			...
			$module->add_setup('editor.addButton("image_picker",' . JSON::encodeJavaScript($image_picker_button) . ");\n");
