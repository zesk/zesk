...
class View_Image extends View {
	...
	protected function source_directory() {
		return $this->option('root_directory', $this->application->documentRoot());
	}

	protected function cache_directory() {
		$directory = $this->option('cache_directory', path($this->application->documentRoot(), '/cache/images/'));
	...
	protected function cache_url_prefix() {
		...
		$doc_root = realpath($this->application->documentRoot());
	...
	private function scale_image($source) {
		...
		$this->debugLog("\$sourceFile = $source");
		$this->debugLog('$this->source_directory()  = ' . $this->source_directory());
		...
		$this->callHook('scale_path', $target_full_path);
		if (self::$debug) {
		...
		if (Image_Library::factory($this->application)->imageScale($source, $target_full_path, $this->options)) {
			[$this->width, $this->height] = getimagesize($target_full_path);
	...
	public function render(): string {
		...
		if (!file_exists($path) || is_dir($path)) {
			$this->debugLog("Image path not found: $path");
	...
	public static function scaled_widget(Application $application, $width = false, $height = false, $alt = '', array $options = []) {
		...
		$w->setRequest($application->request() ?? Request::factory($application, 'http://test/'));
