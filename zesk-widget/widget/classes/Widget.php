...
class Widget extends Hookable {
	...
	private function _setChild(string $id, Widget $child, bool $first = false): void {
		...
		$this->callHookArguments('child', [$child], $id);
		if ($this->_initialize) {
			$child->initialize();
		}
	}
	...
	public function is_visible() {
		...
		return $this->callHookArguments('visible', [], true);
	}
			...
			if ($this instanceof Control) {
				...
				if ($target) {
					...
					if (($valid = $this->validate()) === true) {
						if (($valid = $this->callHookArguments('validate', [], true)) === true) {
							$do_render = $this->submit();
						}
					} else {
						$do_render = $this->callHookArguments('validate_failed', [], null);
					}
				}
			}
	...
	final public function children_hook_array($hooks, array $arguments): void {
		$this->callHookArguments($hooks, $arguments);
		...
		foreach ($children as $child) {
			...
			$child->callHookArguments($hooks, $arguments);
		}
	}
	...
	private function save_new_value($new_value): void {
		...
		$this->callHook('loaded;model_changed');
		if ($this->object && $this->traverse) {
			$this->object->callHook('control_loaded', $this);
		}
	}
	...
	protected function model(): Model {
		$model = $this->callHook('model_new', $this);
		if (!$model instanceof Model) {
			$model = $this->application->factory(__NAMESPACE__ . '\\' . 'ORM', $this->application);
			$model = $this->callHook('model_alter', $model);
		}
		return $model;
	}
	...
	public function render(): string {
		...
		$this->content = $this->callHookArguments('render_alter', [
			$this->content,
		], $this->content);
		return $this->content;
	}
