...
class Control_Login extends Control_Edit {
	...
	public function validate(): bool {
		...
		if ($failed) {
			...
			$user->callHook('login_failed', $this);
			if ($this->preferJSON()) {
				$this->json([
					'status' => false,
					'errors' => $this->errors(),
		...
		if ($user->callHookArguments('login', [
			$this,
		], true)) {
	...
	public function default_submit(): bool {
		...
		if (URL::is($uref) && !URL::isSameServer($uref, $this->request->url())) {
			$uref = false;
		}
		if (!$uref) {
	...
	public function submit(): bool {
		if ($this->user instanceof User) {
			...
			$result = $this->callHookArguments('submit', [], null);
			if ($result !== null) {
				if (is_array($result)) {
					$this->json($result);
				}
		...
		$result = $this->callHookArguments('submit_failed', [], null);
		if ($result !== null) {
			if (is_array($result)) {
				$this->json($result);
			}
