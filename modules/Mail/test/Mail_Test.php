 ...
 (=?iso-8859-8?b?7eXs+SDv4SDp7Oj08A==?=)', true,
       ...
       =?ISO-8859-1?Q?b?)', false,
		...
		foreach ($headers as $i => $header) {
			...
			if ($result) {
				$this->assertNotEquals(0, count(Mail::headerCharsets($test)));
			} else {
				$this->assertEquals(0, count(Mail::headerCharsets($test)));
			}
		}
	}
...
<a href="http://www.example.com/unsubscribe?email={email}">Unsubscribe</a>', 'HTML Failed: ' . $result['body_html']);
	...
	public function test_is_email($email): void {
		$this->assertTrue(is_email($email), "$email is not an email");
	}
