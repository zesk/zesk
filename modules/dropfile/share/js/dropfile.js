...
;(function(exports, $) {
	...
	document_target_class = zesk.get('modules.' + plugin_name + '.document_target_class', 'dropfile-document-target'),
	/*
	...
	clean_path = function (n) {
		return n.toString().replace(/\\/g, "/").rright("/");
	},
	defaults = {
		column: "file",
		url: null,
		...
		type_error: function (files) {
			$.each(files, function () {
				zesk.message(__("DropFile:=The file {file} was not uploaded because it is the wrong type. ({type})", { 
	...
	$.extend(DropFile.prototype, {
		setup_file_input: function () {
			...
			$input = this.$input,
			$iframe;
			if ($('form input', this.$element).length > 0) {
				return;
			}
			if (false && this.remove) { // TODO
				this.$element.append('<a class="dropfile-remove-button"><span class="glyphicon glyphicon-remove"></span></a>');
				$('.dropfile-remove-button', this.$elemnt).on("click", function () {
					$('img', this.$element).remove();
					$('.dropfile-value', $this.element).val("");
					$(this).remove();
				});
			}
			if (this.accept) {
			...
			this.$element.append('<form action="' + this.url + '" method="post" target="dropfile-' + this.column + '" enctype="multipart/form-data"><input type="file" name="' + this.column + '"' + accept + ' /><iframe name="dropfile-' + this.column + '" style="display: none"></iframe><input type="hidden" name="ok" value="ok" /></form>');
		...
		upload_files: function (files) {
			var i, fd, type_errors = [], max_errors = [], nfiles = 0;
		...
		progress_start: function (file) {
			...
			name = file.name;
			this.filename = name;
			this.$element.prepend('<div class="progress"><div class="progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%;"><span class="sr-only">60%</span></div><div class="dropfile-name">'+name+'</div></div>');
			$(this.$element).removeClass(document_target_class).removeClass(this.target_class);
