...
use zesk\Database as DatabaseBase;
 ...
 *
 */
class Database extends \zesk\Database implements Database_Interface {
	/**
	...
	protected function initialize(): void {
		$this->parser = new Database_Parser();
		$this->sql = new Database_SQL();
		$this->data_type = new Database_Type();
	}
	...
	public function internalConnect(): void {
		...
		try {
			$this->conn = new SQLite3($path, $flags, $encryption_key);
		} catch (Throwable $t) {
	...
	public function databaseTable(string $table_name): Database_Table {
		...
		$sql = $this->resultRowColumn($statement, 'sql');
		$sql .= ";\n";
		...
		$statement->bindParam(':name', $table, SQLITE3_TEXT);
		$indexes_sql = $this->resultRow($statement);
