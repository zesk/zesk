...
use zesk\SQLParser as BaseParser;
...
class SQLParser extends BaseParser {
	 ...
	 * @return multitype:
	 */
	private function parse_column_options_sql(Table $table, $sql_type, $column_options) {
		static $patterns = [
	 ...
	 */
	private function parse_column_sql(Table $table, $sql) {
		$columns_matches = [];
		if (!preg_match_all(SQLITE_PATTERN_COLUMN_LIST, $sql, $columns_matches, PREG_SET_ORDER)) {
			throw new ParseException('Unable to parse table {tableName} column definition: {sql}', [
		...
		foreach ($columns_matches as $col_match) {
			 ...
			 */
			$options = [];
			$column_name = StringTools::unquote($col_match[2], SQLITE_COLUMN_QUOTES);
			...
			$size = Types::toInteger(StringTools::unquote($col_match[4], '()'));
			if ($size > 0) {
				$options['size'] = $size;
			}
			...
			$col = new Column($table, $column_name, $options);
	...
	public function createIndex(Table $table, $sql) {
		$indexes = self::parse_index_sql($table, $sql);
		return ArrayTools::first($indexes);
	}
	 ...
	 * @return string
	 */
	private function parse_index_sql(Table $table, $sql) {
		/*
		...
		foreach ($index_matches as $index_match) {
			...
			$table_name = StringTools::unquote($table_name, SQLITE_COLUMN_QUOTES);
			if ($table_name !== $original_table_name) {
				continue;
			}
			$index_name = StringTools::unquote($index_name, SQLITE_COLUMN_QUOTES);
			$columns = trim($columns);
			if (!preg_match_all(SQLITE_PATTERN_INDEX_COLUMN_LIST, "$columns,", $column_matches, PREG_PATTERN_ORDER)) {
				throw new ParseException('Unable to parse SQLite3 {table} columns {index_name}: {raw_columns}', [
					'table' => $table_name, 'index_name' => $index_name, 'raw_columns' => $columns,
				]);
			}
			$column_matches = StringTools::unquote($column_matches[1], SQLITE_COLUMN_QUOTES);
			$unique = strcasecmp($unique, 'unique') ? Index::TYPE_UNIQUE : Index::TYPE_INDEX;
			$indexes[] = $index = new Index($table, $index_name, $unique);
	...
	private static function tips(&$sql) {
		...
		if (preg_match_all(SQLITE_PATTERN_TIP_ALTER, $sql, $tip_matches, PREG_SET_ORDER)) {
			foreach ($tip_matches as $tip_match) {
				...
				$col = StringTools::unquote($column, SQLITE_COLUMN_QUOTES);
				if ($plus_minus === '+') {
	 ...
	 * @todo move to parent class
	 */
	private function apply_tips(Table $table, array $tips): void {
	 ...
	 * The money
	 *
	 * @see \zesk\MySQL\SQLParser::createTable()
	 * Parses CREATE TABLE for MySQL and returns a Table
	 */
	public function createTable(string $sql): Table {
		...
		if (!preg_match(SQLITE_PATTERN_CREATE_TABLE, $sql, $matches)) {
			throw new ParseException('Unable to parse CREATE TABLE starting with: {sql}', [
				'sql' => substr($sql, 0, 99),
			]);
		}

		$table = StringTools::unquote($matches[1], SQLITE_TABLE_QUOTES);
		/*
		 * Parse table options (end of table declaration)
		 */
		$table_options = self::create_table_options($matches[3]);

		$table = new Table($this->database, $table);
