...
class SelectTest extends DatabaseUnitTest {
	...
	public function test_main(): void {
		...
		$select->setOrderBy(Types::toList('A;B;C'));
		...
		$this->assertInstanceOf(Base::class, $select->database());
		...
		$result = strval($x);
		$valid_result = 'SELECT ID FROM `Select` WHERE (`ID` = 1 OR `ID` = 2 OR `ID` = 3 OR `ID` = 4)';
		...
		$result = strval($x);
		$valid_result = 'SELECT ID FROM `Select` WHERE (`ID` != 1 AND `ID` != 2 AND `ID` != 3 AND `ID` != 4)';
		...
		$result = strval($x);
		$valid_result = 'SELECT ID FROM `Select` WHERE (SUM(Total)!=1 AND SUM(Total)!=2 AND SUM(Total)!=3 AND SUM(Total)!=4)';
