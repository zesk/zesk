...
abstract class Contact_Import extends Options {
	 ...
	 */
	private $tag = null;
	function __construct(Interface_Process $process, $filename, array $options = array()) {
	 ...
	 * @return \zesk\unknown
	 */
	function register_label(Contact_Info $object, $name) {
	 ...
	 * @return boolean
	 */
	function import_row($row_index, array $row) {
		$this->reset();
		$map = $this->header_map();
