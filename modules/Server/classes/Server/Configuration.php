...
abstract class Server_Configuration extends Hookable {
	...
	final public function register_types(array $variable_types, array $variable_defaults, $check_conflict = true) {
		foreach ($variable_types as $name => $type) {
			...
			if ($check_conflict && ($existing_type = $this->variable_types[$name] ?? $type) !== $type) {
	...
	final public function check_type_before($name) {
		try {
			return $this->validate_type_before($this->variable_types[strtolower($name)] ?? null, $name, $this->option($name, $this->application->configuration->get($name)));
		} catch (Exception $e) {
			return false;
		}
	}
	...
	final public function check_type_after($name) {
		...
		try {
			return $this->validate_type_after($this->variable_types[strtolower($name)] ?? null, $name, $this->option($name, $this->application->configuration->get($name)));
		} catch (Exception $e) {
			return false;
		}
	}
