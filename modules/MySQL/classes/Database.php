...
class Database extends \zesk\Database {
	 ...
	 * @var array
	 */
	private static array $mysql_default_attributes = [
	...
	final public function internalConnect(): void {
		...
		$this->Database = $database = substr($parts['path'] ?? null, 1);
	...
	final protected function basic_types_compatible($t0, $t1) {
		...
		static $map = [
			'float' => 'double', 'integer' => 'double', 'boolean' => 'integer',
		];
		return $map[$t0] ?? $t0 === $map[$t1] ?? $t1;
	}
	...
	public function releaseLock(string $name): void {
		...
		if (intval($result) !== 1) {
			throw new Exception_Semantics('Released lock {name} FAILED (raw_result={raw_result}): ', [
				'name' => $name, 'backtrace' => debug_backtrace(DEBUG_BACKTRACE_PROVIDE_OBJECT),
	...
	public function bytesUsed(string $table_name = ''): int {
		if ($table_name !== '') {
			if (!$this->tableExists($table_name)) {
				throw new Database_Exception_Table_NotFound($this, $table_name);
			}
			return $this->queryOne("SHOW TABLE STATUS LIKE '$table_name'", 'Data_length');
		} else {
			$total = 0;
