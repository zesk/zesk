	...
	public function current(): mixed {
		...
		if ($this->_value) {
			if (!array_key_exists($this->_value, $this->_row)) {
				throw new Exception_Semantics('Query result does not contain value "{value}"', ['value' => $this->_value, ]);
			}
	...
	public function key(): mixed {
		if ($this->_key) {
			if (!array_key_exists($this->_key, $this->_row)) {
				throw new Exception_Semantics('Query result does not contain key {key}', ['key' => $this->_key, ]);
			}
