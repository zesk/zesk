...
class Control_Time_Zone extends Control_Select {
	...
	public function initialize(): void {
		$query = $this->application->ormRegistry('zesk\\Time_Zone')->querySelect()->where($this->where);
		if ($this->prefixes_only()) {
			$query->addWhat('*Name', 'DISTINCT LEFT(Name,LOCATE(\'/\',Name)-1)');
			$tzs = ArrayTools::clean(ArrayTools::suffixKeys($query->toArray('Name', 'Name'), '/'), '');
			$this->control_options($tzs);
		} else {
			$query->addWhat('Name', 'Name');
