...
class Command_Translate extends Command_Base {
	...
	protected function run() {
		...
		$source_language_file = $this->option("language-file", $zesk->configuration->path_get("Module_PolyGlot::source_file"));
		if (!$source_language_file) {
			$this->usage("Need a source --language-file to determine source strings");
		}
		$destination = $this->option("destination", $zesk->configuration->path_get("Locale::auto_path"));
		if (!$destination) {
		...
		$source_language = $this->option("source", Locale::language());
		...
		$target_translations = $zesk->load($target_file);
		...
		$translation_file = $zesk->load($source_language_file);
	...
	/**
	 * Remove tokens from the phrase so they are not automatically translated (and left alone by remote service)
	 * 
