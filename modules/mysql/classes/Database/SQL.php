...
class Database_SQL extends \zesk\Database_SQL {
	...
	public function alter_table_index_add(Database_Table $t, Database_Index $index) {
		...
		throw new Exception_Invalid("{class}::sql_alter_table_index_add({table}, {indexType}, ...): Invalid index type {indexType}", compact("indexType", "table") + [
			"class" => __CLASS__,
		]);
	}
	...
	public function alter_table_change_column(Database_Table $dbtable, Database_Column $db_col_old, Database_Column $db_col_new) {
		$table = $dbtable->name();
		/*
	...
	public function index_type($table, $name, $type, array $column_sizes) {
		switch ($type) {
			...
			case Database_Index::Primary:
				...
				break;
			default:
				throw new Exception_Invalid("{method}($table, $name, $type, ...): Invalid index type {name}", compact("name") + [
					"method" => __METHOD__,
				]);
		}
		if ($name) {
	...
	public function grant(array $options) {
		...
		if (!isset($options['user']) || !isset($options['pass'])) {
			throw new Exception_Parameter("Need a user and pass option passed to {method}", [
				"method" => __METHOD__,
			]);
		}
		$permutations = [
