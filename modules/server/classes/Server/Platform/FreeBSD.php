...
class Server_Platform_FreeBSD extends Server_Platform_UNIX {
	...
	protected $command_locations = array('pw' => '/usr/sbin', 'sysctl' => '/sbin');
	...
	public function user($user) {
		...
		try {
			...
			return self::$users[$user] = arr::rekey(array(self::f_user_name, "x-password", self::f_user_id, self::f_user_group_id, "x-class", "x-change", "x-expire", self::f_user_full_name, self::f_user_home, self::f_user_shell), explode(":", $result, 9));
	...
	public function group($group) {
		...
		try {
			...
			// publish:*:1001:
			$data = arr::rekey(array(self::f_group_name, null, self::f_group_id, self::f_group_members), explode(":", $result, 4));
