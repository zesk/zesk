 ...
 *
 * Customized by kent -at- marketruler.com for ruler
 */
...
function Datepicker() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		defaultDate: null, // Used when field is blank: actual dat13e,
...
$.extend(Datepicker.prototype, {
	...
	_setRange: function (inst, date,endDate) {
		this._setDate(inst, date, endDate);
		this._updateDatepicker(inst);
	...
	_selectRangeMonth: function (id, y, m) {
		var target = $(id);
		var inst = $.data(target[0], PROP_NAME);
		if (!inst) return;
		if (!this._get(inst, 'rangeSelect')) return;
		if (inst.rangeStart) return;
		var date = new Date(); date.setDate(1); date.setMonth(m); date.setYear(y);
		var endDate = new Date(); endDate.setDate(1); endDate.setMonth(m); endDate.setYear(y); endDate.setDate(this._getDaysInMonth(y,m));
		this._setRange(inst,date,endDate);
	},
...
//			}
		...
		var prev = '<div class="ui-datepicker-prev">' + (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
			...
			(hideIfNoPrevNext ? '' : '<label>' + prevBigText + '</label><label>' + prevTextDisabled + '</label>')) + '</div>';
		...
		var next = '<div class="ui-datepicker-next">' + (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
			...
			(hideIfNoPrevNext ? '' : '<label>' + nextTextDisabled + '</label><label>' + nextBigText + '</label>')) + '</div>';
		...
		var html = (prompt ? '<div class="' + this._promptClass + '">' + prompt + '</div>' : '') +
			...
			currentText + '</a></div>' : '') + (!this._get(inst, 'prevNextAlone') ? (isRTL ? prev : next) : '') + '</div>';
			...
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						var printTime = printDate.getTime();
						html += '<td class="ui-datepicker-days-cell' +
							((dow + firstDay + 6) % 7 >= 5 ? ' ui-datepicker-week-end-cell' : '') + // highlight weekends
							(otherMonth ? ' ui-datepicker-other-month' : '') + // highlight days from other months
							(printTime == selectedDate.getTime() && drawMonth == inst.selectedMonth ?
							' ui-datepicker-days-cell-over' : '') + // highlight selected day
							(unselectable ? ' ' + this._unselectableClass : '') +  // highlight unselectable days
							(otherMonth && !showOtherMonths ? '' : ' ' + daySettings[1] + // highlight custom dates
							(printTime >= currentDate.getTime() && printTime <= endDate.getTime() ?  // in current range
							' ' + this._currentClass : '') + // highlight selected day
							(printTime == today.getTime() ? ' ui-datepicker-today' : '')) + '"' + // highlight today (if different)
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		var monthHtml = '';
		// month selection
		var clickMonth = this._get(inst, 'clickMonth');

		if (clickMonth) {
			var minDate = this._getMinMaxDate(inst, 'min', true);
			var maxDate = this._getMinMaxDate(inst, 'max');
			var d = new Date(); d.setDate(1); d.setMonth(drawMonth); d.setYear(drawYear);
			if (d > maxDate) {
				clickMonth = false;
			} else {
				d.setDate(this._getDaysInMonth(drawYear, drawMonth));
				if (d < minDate) {
					clickMonth = false;
				}
			}
		...
		if (secondary || !this._get(inst, 'changeMonth'))
			if (clickMonth) {
				monthHtml += '<a onclick="jQuery.datepicker._selectRangeMonth(\'#'+inst.id+'\','+drawYear+','+drawMonth+')">' + monthNames[drawMonth] + '&#xa0;' + drawYear + '</a>';
			} else {
