...
class Module extends \zesk\Module {
	...
	private function _less_path() {
		return $this->option('less_path', path($this->application->documentRoot(), '/less/' . $this->style_name() . '.less'));
	}
	...
	private function _full_css_path() {
		return path($this->application->documentRoot(), $this->_css_path());
	}
	...
	public function css_theme_dirty(): void {
		File::unlink(path($this->application->documentRoot(), $this->_css_path()));
	}
	...
	private function _compile_css_theme(array $variables = []): void {
		...
		$types = $variables + $this->callHookArguments('less_variables', [], []);
		$variables = [];
