...
class PHP_Inspector {
	 ...
	 */
	static function factory(Application $application, $file) {
	 ...
	 * @param string $file
	 */
	function __construct(Application $application, $file) {
		File::depends($file);
	 ...
	 */
	function reflection_class($class) {
		...
		while ($index < $this->tokens_length) {
			...
			if (!in_array($type, $ignore_tokens)) {
				if (in_array($type, $capture_tokens)) {
					$capture .= $text;
				} else if (in_array($type, $stop_tokens)) {
					$index--;
					return $capture;
				} else {
					return $capture;
