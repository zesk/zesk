<?php
/**
* $URL: https://code.marketacumen.com/zesk/trunk/classes/View/Date/Range.php $
* @package zesk
* @subpackage default
...
class View_Date_Range extends View {
	...
	function render() {
		...
		if ($start_date->equals($end_date)) {
			...
			$end_format = "";
		} else if (!$start_date->equalYears($end_date)) {
		...
		} else if (!$start_date->equalMonths($end_date)) {
			$start_format = "{MMMM} {DDD}";
			$end_format = " - {MMMM} {DDD}$end_date_year";
		} else if ($start_date->getDay() == 1 && $end_date->isLastDayOfMonth()) {
